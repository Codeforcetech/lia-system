...
import { GenerateForm } from "@/app/customers/[id]/generate/GenerateForm";
...
export default async function GeneratePage({
  ...
  searchParams: Promise<{ purpose?: string; tone?: string }>;
}) {
  const { id } = await params;
  const { purpose: qpPurpose, tone: qpTone } = await searchParams;
      ...
      <section className="lia-card px-5 py-6 sm:px-6 sm:py-7">
        <div className="flex items-start justify-between gap-4">
          <div className="space-y-3">
            ...
            <p className="text-sm font-normal leading-relaxed text-liaInk-muted">
              {customer.name} さんへ
            </p>
            <p className="lia-prose-soft max-w-md">
              メモや来店の記録を見ながら、送りやすい3案をご用意します。気に入ったものをコピーして、そのままLINEへ。
            </p>
          </div>
